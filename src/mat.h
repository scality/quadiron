...
{
 public:
  Mat(MG<T> *mg, int n_rows, int n_cols);
  ~Mat();
...
template <typename T>
Mat<T>::Mat(MG<T> *mg, int n_rows, int n_cols)
{
  this->mg = mg;
  this->n_rows = n_rows;
  this->n_cols = n_cols;
...
void Mat<T>::mul_row(int row, T factor)
{
  ...
  for (j = 0; j < n_cols; j++) {
    set(row, j, mg->mul(get(row, j), factor));
  }
}
...
void Mat<T>::add_rows(int src_row, int dst_row, T factor)
{
  ...
  for (j = 0; j < n_cols; j++) {
    set(dst_row, j,
        mg->add(get(dst_row, j),
                mg->mul(get(src_row, j), factor)));
  }
}
...
void Mat<T>::reduced_row_echelon_form(void)
{
  ...
  for (j = 0; j < n_cols && num_pivots < n_rows; j++) {  // For each column
    ...
    mul_row(pivot_row, mg->div(1, get(pivot_row, j)));

    // Eliminate rows below
    for (i = pivot_row + 1; i < n_rows; i++)
      add_rows(pivot_row, i, mg->neg(get(i, j)));
  }
  ...
  for (int i = num_pivots - 1; i >= 0; i--) {
    ...
    for (int j = i - 1; j >= 0; j--)
      add_rows(i, j, mg->neg(get(j, pivot_col)));
  }
}
...
void Mat<T>::inv(void)
{
  ...
  Mat<T> tmp(this->mg, n_rows, n_cols * 2);
  for (i = 0; i < n_rows; i++) {
    for (j = 0; j < n_cols; j++) {
      tmp.set(i, j, get(i, j));
...
void Mat<T>::mul(Vec<T> *output, Vec<T> *v)
{
  ...
  for (i = 0; i < n_rows; i++) {
    for (j = 0; j < n_cols; j++) {
      T x = mg->mul(get(i, j), v->get(j));
      if (0 == j)
        output->set(i, x);
      else
        output->set(i, mg->add(output->get(i), x));
    }
  }
}
...
void Mat<T>::cauchy()
{
  ...
  for (i = 0; i < n_rows; i++) {
    for (j = 0; j < n_cols; j++) {
      set(i, j, mg->inv(mg->add(i, (j + n_rows))));
    }
  }
  ...
  for (j = 0; j < n_cols; j++) {
    for (i = 0; i < n_rows; i++) {
      set(i, j, mg->div(get(i, j), get(0, j)));
    }
  }
  // convert 1st element of each row to 1
  for (i = 1; i < n_rows; i++) {
    for (j = 0; j < n_cols; j++) {
      set(i, j, mg->div(get(i, j), get(i, 0)));
    }
  }
}
